...
public class FrmNhanVien extends JFrame {
	...
	public ResultSet timkiem(String name) throws Exception {
		String sql = " select * from tbl_NhanVien where hoten like N'%" + name + "%'";
...
*/
	public FrmNhanVien() {
		...
		contentPane.add(etMNV);etMNV.setColumns(10);
		etHT = new JTextField();
		...
		etLuong.setColumns(10);JButton btThem = new JButton("Th\u00EAm");
		btThem.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				try {
					...
					nvbo.them( manv, hoten,
					gioitinh, ngaySinh, sdt, diachi, ngaylamviec, luong);
					table.setModel(nvbo.napbang("nhanvien"));
					} else {
			...
			public void mouseClicked(MouseEvent arg0) {
				try {
					...
					if(kiemtra(manv).next()) {
						...
						nvbo.sua( manv, hoten, gioitinh,
						ngaySinh, sdt, diachi, ngaylamviec, luong);
						table.setModel(nvbo.napbang("nhanvien"));
					}
				}catch (Exception e) {
					e.printStackTrace();
		...
		contentPane.add(etTK);etTK.setColumns(10);
		...
		lblNewLabel_10.setForeground(Color.RED);lblNewLabel_10.setFont(new Font("Tahoma", Font.PLAIN, 35));
